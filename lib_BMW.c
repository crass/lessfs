 ...
 *   Blue Midnight Wish is designed to be forever free for all uses, so you can
 *   use it for whatever product you are designing (commercial or under GPL
 *   license).
...
unsigned char *Hash (int hashbitlen, const BitSequence *data,
                 DataLength databitlen)
{
   HashReturn Status;
   hashState state;
   BitSequence *hashval=NULL;

   hashval=(BitSequence *)s_malloc(hashbitlen/8);
   Status = Init(&state, hashbitlen);
   if (Status != SUCCESS) return NULL;
   Status = Update (&state, data, databitlen*8);
   if (Status != SUCCESS) return NULL;
   Status = Final (&state, hashval);
   if (Status != SUCCESS) return NULL;

   return (unsigned char *)hashval;
}
